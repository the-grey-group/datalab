...
def real_mongo_client():
    ...
    try:
        ...
        pytest.skip("No real MongoClient found")
...
def client(real_mongo_client):
    ...
    try:
        ...
        if TEST_DATABASE_NAME in databases:
            raise RuntimeError(
                f"Not running tests over the top of existing database named {TEST_DATABASE_NAME!r} at {MONGO_URI}; try dropping it before running the tests if this is desired."
            )
        LOGGER.warning(f"Using real database at {MONGO_URI}")
